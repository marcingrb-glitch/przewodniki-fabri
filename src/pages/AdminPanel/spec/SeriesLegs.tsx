...
export default function SeriesLegs({ seriesProductId, seriesProperties }: Props) {
  ...
  const fetchAll = async () => {
    ...
    // Fetch allowed_chest relations to get chest IDs
    ...
    const allowedChestIds = (allowedChestRels ?? []).map(r => r.target_product_id).filter(Boolean) as string[];

    const [legsRes, chestsRes, seriesAutomatsRes, globalAutomatsRes, extrasRes, allowedLegsRes] = await Promise.all([
      supabase.from("products").select("id, code, name, properties, colors")
        .eq("category", "leg").eq("is_global", true).order("code"),
      allowedChestIds.length > 0
        ? supabase.from("products").select("id, code, name, properties")
            .eq("category", "chest").in("id", allowedChestIds).order("code")
