...
interface WarehouseGuidePdfPreviewProps {
  decoded: DecodedSKU | null;
  width?: number;
}

export default function WarehouseGuidePdfPreview({
  decoded,
  width = 500,
}: WarehouseGuidePdfPreviewProps) {
  ...
  useEffect(() => {
    ...
    async function render() {
      ...
      setError(null);
      try {
        const blob = await generateWarehouseGuidePDF(debouncedDecoded);
        ...
        console.error("WarehouseGuidePdfPreview render error:", e);
        if (!cancelled) {
          setRendering(false);
