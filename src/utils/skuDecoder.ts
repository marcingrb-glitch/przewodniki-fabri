...
export async function decodeSKU(parsed: ParsedSKU): Promise<DecodedSKU> {
  ...
  const [
    ...
    seriesId
      ? supabase.from("seats_sofa").select("code, frame, front, center_strip, default_finish, allowed_finishes, type, spring_type, model_name, frame_modification").eq("code", seatCode).eq("series_id", seriesId).maybeSingle()
      : Promise.resolve({ data: null }),
