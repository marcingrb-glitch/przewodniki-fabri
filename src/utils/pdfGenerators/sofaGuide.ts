...
export function generateSofaGuidePDF(decoded: DecodedSKU): Blob {
  ...
  y = addTable(doc, y,
    ["Siedzisko", "Stelaz", "Pianka", "Front", "Pasek srodek"],
    [[
  ...
  // Optional: Wałek
  if (decoded.walek) {
    y = addSectionTitle(doc, "WALKI", y);
    y = addTable(doc, y,
      ["Walek", "Typ", "Wykonczenie"],
  ...
  if (hasPufa || hasFotel) {
    if (hasPufa) {
      y = addInfoBox(doc, y, "Do zamowienia jest PUFA");
    }
    if (hasFotel) {
      y = addInfoBox(doc, y, "Do zamowienia jest FOTEL");
    }
  }
